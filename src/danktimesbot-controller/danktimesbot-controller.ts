...
export class DankTimesBotController implements IDankTimesBotController {
    ...
    public onPluginWantsToParseScoreInput(input: string, userScore: number | undefined = undefined,
        previousInput: number | undefined = undefined, previousWin: number | undefined): number | null {
        return this.util.parseScoreInput(input, userScore, previousInput, previousWin);
    }
    ...
    public onPluginWantsToSendFile(chatId: number, filePath: string, replyToMessageId: number,
        forceReply: boolean, caption = "", type: "photo" | "video" | "audio" | "voice" | "document" = "photo"): Promise<void | TelegramBot.Message> {
