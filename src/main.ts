...
tgClient.retrieveBotName().then(() => {
  tgClient.registerCommand(new TelegramBotCommand("addtime", "adds a dank time. format: [hour] [minute] [points] [text1] [text2] etc.", commands, commands.addTime, true));
  tgClient.registerCommand(new TelegramBotCommand("danktimes", "shows the user-specified dank times", commands, commands.dankTimes));
  tgClient.registerCommand(new TelegramBotCommand("help", "shows the available commands", commands, commands.help));
  tgClient.registerCommand(new TelegramBotCommand("leaderboard", "shows the leaderboard", commands, commands.leaderBoard));
  tgClient.registerCommand(new TelegramBotCommand("releases", "shows the release log", commands, commands.getReleaseLog));
  tgClient.registerCommand(new TelegramBotCommand("removetime", "removes a dank time. format: [hour] [minute]", commands, commands.removeTime, true));
  tgClient.registerCommand(new TelegramBotCommand("reset", "resets the scores", commands, commands.resetChat, true, true));
  tgClient.registerCommand(new TelegramBotCommand("setdailyrandomfrequency", "sets the number of random dank times per day. format: [number]", commands, commands.setDailyRandomTimes, true));
  tgClient.registerCommand(new TelegramBotCommand("setdailyrandompoints", "sets the points for random daily dank times. format: [number]", commands, commands.setDailyRandomTimesPoints, true));
  tgClient.registerCommand(new TelegramBotCommand("setmultiplier", "sets the multiplier for the score of the first user to score. format: [number]", commands, commands.setMultiplier, true));
  tgClient.registerCommand(new TelegramBotCommand("settimezone", "sets the time zone. format: [timezone]", commands, commands.setTimezone, true));
